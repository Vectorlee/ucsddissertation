...
%\pp{For camera ready, we need a dataset section. See comment in latex below this.}
% A dataset table that looks like:
%
% source         start        stop      whatwasit   pub/priv    #things
% a
% b
% c
% Also category info.
...
%\note{We need to state why our dataset is representative? It is one of my big
%concern. For example, our spam data doesn't include the popular spam blacklist
%like CBL, Spamhaus, SORBS}
...
\emphpar{\feedetiprep\ Service} This commercial service provides an
    ...
    %Each IP indicator
    %includes information on the associated attack, attack category, and timing
    %info. %We collected this source hourly during our collection window.

\emphpar{Public Blacklists and Reputation Feeds} We collected indicators from public
...
collection gaps. In this paper, we use the time window from \veryemph{December 1, 2017}
to \veryemph{July 20, 2018} for most of the analyses, as we have the largest number
...
%One limitation of our study is that, while some of our data arises from
%commercial sources, much of it is public data. Our results will be of interest to
%commercial threat intelligence users to the extent their specific \ti\ sources have the same %characteristics as the sources we use.
...
In this paper, we choose a 30-day valid period for all the indicators we collected from event feeds---the same valid period
...
query option.
%One can think of an event feed as a snapshot feed with every indicators
%having a minimum expiration time: at a certain moment there is no members in
%the feed, we only ask for data in a time window.
